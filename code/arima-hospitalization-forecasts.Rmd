...
```{r}
hosp_arima_forecast_function <- function(case_data_source, forecast_date, case_data_start_date, data_issue_date) {
...
if(case_data_source == "Report") {
 ...
 model_cases_by_source <- merge_all_ts[c("target_end_date", "temp_col")] %>% as_tsibble() %>%
...
all_cases_by_date <-  merge_all_ts[c("target_end_date", "temp_col")] %>%
  bind_rows(select(tsibble(new_case_by_source_data), target_end_date, temp_col = `.mean`))
...
hosp_arima_forecast_function(case_data_source = "Report",
                             hosp_data_start_date <- as.Date("2020-08-01"),
                             case_data_start_date <- as.Date("2020-03-01"),
                             forecast_date <- as.Date("2021-06-01"))

#hosp_arima_forecast_function(case_data_source = "Test") ,
