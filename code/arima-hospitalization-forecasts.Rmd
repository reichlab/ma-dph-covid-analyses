...
Here is a model with a SQRT transformation that doesn't seem to do what we want exactly (note the funny intervals at the end of the forecast period):
```{r}
#write model for ARIMA with only hospitalizations 
fit_hosp_alone_sqrt <- ts_hosp %>%
  filter(target_end_date >= hosp_data_start_date) %>%
  model(ARIMA(sqrt(hosp_val))) 
