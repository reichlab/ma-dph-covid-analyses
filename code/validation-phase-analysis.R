...
test_case_final <- read.csv("../csv-data/MA-DPH-csvdata-covid-2022-02-02.csv") %>%  #chose latest issue date from csv files
  mutate(issue_date = data_issue_date) %>% 
  mutate(target_end_date = ymd(substr(Date, 1, 10))) %>% 
  filter(
    target_end_date <= forecast_date, 
    target_end_date >= case_data_start_date) %>% 
  select(target_end_date, test_case = Positive.New) %>% 
  as_tsibble() %>% 
  tsibble::fill_gaps() 
