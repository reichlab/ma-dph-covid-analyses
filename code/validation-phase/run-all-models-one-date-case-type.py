...
if __name__ == "__main__":
	...
	# add trend/seasonal decomposition
	data = st_decomp(target_var='case', data=data)
	# data = st_decomp(target_var='hosp', data=data)
	...
	if model_group == 'SARIX':
		...
		for i in range(sari_variations.shape[0]):
			...
			if smooth_case:
				modeled_vars = ['case_trend', 'hosp']
			else:
				modeled_vars = ['case', 'hosp']

			diffed_data = diff_df(data, modeled_vars, d, D, 7)

			sarix_fit = sarix.SARIX(
				xy = diffed_data[modeled_vars].dropna().values,
				p = p,
				P = P,
				season_period = 7,
				transform = "none",
		...
		var_variations =  expand_grid({
			'd': [d for d in range(2)],
			'D': [D for D in range(2)]
		})
		
		for i in range(var_variations.shape[0]):
			d = var_variations.d.values[i]
			D = var_variations.D.values[i]

			diffed_data = diff_df(sm_data, ['case', 'hosp'], d, D, 7)
			...
			preds = model_fit.forecast_interval(diffed_data.values[-lag_order:], steps=effective_horizon_rel_obs)
			pred_means = preds[0]
			...
			# invert differencing operation
			pred_qs = inv_diff_pred_q(sm_data, np.transpose(pred_qs), 'hosp', d, D, 7)
